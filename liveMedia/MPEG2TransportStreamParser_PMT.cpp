...
::parsePMT(PIDState_PMT* pidState, Boolean pusi, unsigned numDataBytes) {
  ...
  Log.Error(__FILE__, __LINE__, "\tProgram Map Table\n");
#endif
  ...
  do {
    ...
    if (table_id != 0x02) {
#ifdef DEBUG_ERRORS
      Log.Error(__FILE__, __LINE__, "MPEG2TransportStreamParser::parsePMT(0x%04x, %d, %d): bad table_id: 0x%02x\n",
...
#ifdef DEBUG_ERRORS
      Log.Error(__FILE__, __LINE__, "MPEG2TransportStreamParser::parsePMT(0x%04x, %d, %d): Bad section_length: %d\n",
...
#ifdef DEBUG_ERRORS
      Log.Error(__FILE__, __LINE__, "MPEG2TransportStreamParser::parsePMT(0x%04x, %d, %d): section_length %d gives us a total size %d that's too large!\n",
...
#ifdef DEBUG_ERRORS
      Log.Error(__FILE__, __LINE__, "MPEG2TransportStreamParser::parsePMT(0x%04x, %d, %d): program_number %d does not match the value %d that was given to us in the PAT!\n",
	      pidState->PID, pusi, numDataBytes, program_number, pidState->program_number);
#endif
      break;
    }
#ifdef DEBUG_CONTENTS
    Log.Error(__FILE__, __LINE__, "\t\tprogram_number: %d\n", program_number);
    ...
    Log.Error(__FILE__, __LINE__, "\t\tPCR_PID: 0x%04x\n", PCR_PID);
#else
    skipBytes(5);
#endif
...
#ifdef DEBUG_CONTENTS
    Log.Error(__FILE__, __LINE__, "\t\tprogram_info_length: %d\n", program_info_length);
#endif
    unsigned endOfDescriptors = curOffset() + program_info_length;
    if (endOfDescriptors + 4/*CRC*/ - startPos > numDataBytes) {
#ifdef DEBUG_ERRORS
      Log.Error(__FILE__, __LINE__, "MPEG2TransportStreamParser::parsePMT(0x%04x, %d, %d): program_info_length %d gives us a total size %d that's too large!\n",
    ...
    while (curOffset() <= endPos - 4/*for CRC*/ - 5/*for mapping fields*/) {
      ...
      Log.Error(__FILE__, __LINE__, "\t\tstream_type: 0x%02x (%s); elementary_PID: 0x%04x; ES_info_length: %d\n",
	      stream_type, streamTypeDesc == NULL ? "???" : streamTypeDesc, elementary_PID, ES_info_length);
#endif
      endOfDescriptors = curOffset() + ES_info_length;
      if (endOfDescriptors + 4/*CRC*/ - startPos > numDataBytes) {
#ifdef DEBUG_ERRORS
	Log.Error(__FILE__, __LINE__, "MPEG2TransportStreamParser::parsePMT(0x%04x, %d, %d): ES_info_length %d gives us a total size %d that's too large!\n",
...
#ifdef DEBUG_CONTENTS
#define pDesc(str) do { Log.Error(__FILE__, __LINE__, "\t\t\tdescriptor_tag: 0x%02x (%s); descriptor_length: %d\n",descriptor_tag, (str), descriptor_length); } while (0)
#else
#define pDesc(str)
#endif
...
void MPEG2TransportStreamParser::parseStreamDescriptors(unsigned numDescriptorBytes) {
  while (numDescriptorBytes >= 2/* enough for "descriptor_tag" and "descriptor_length" */) {
...
#ifdef DEBUG_ERRORS
      Log.Error(__FILE__, __LINE__, "MPEG2TransportStreamParser::parseStreamDescriptors() error: Saw descriptor_length %d > remaining bytes %d\n",
...
#ifdef DEBUG_CONTENTS
	Log.Error(__FILE__, __LINE__, "\t\t\t\tflags: 0x%02x (frame_rate_code 0x%1x; MPEG_1_only_flag %d)\n",
	...
	if (MPEG_1_only_flag == 0) {
	  ...
	  flags = get1Byte();
	  Log.Error(__FILE__, __LINE__, "\t\t\t\tprofile_and_level_indication 0x%02x; flags 0x%02x (chroma_format 0x%1x)\n",
...
#ifdef DEBUG_CONTENTS
	u_int8_t flags = get1Byte();
	Log.Error(__FILE__, __LINE__, "\t\t\t\tflags: 0x%02x (layer %d)\n", flags, (flags&0x30)>>4);
#else
	skipBytes(1);
#endif
        numDescriptorBytes -= descriptor_length; parsedDescriptor = True;
	...
	Log.Error(__FILE__, __LINE__, "\t\t\t\tformat_identifier: 0x%08x (%c%c%c%c)\n",
		format_identifier,
		format_identifier>>24, format_identifier>>16, format_identifier>>8, format_identifier);
	if (descriptor_length > 4) {
	  Log.Error(__FILE__, __LINE__, "\t\t\t\tadditional_identification_info: ");
	  for (unsigned i = 4; i < descriptor_length; ++i) Log.Error(__FILE__, __LINE__, "%02x:", get1Byte());
	...
	Log.Error(__FILE__, __LINE__, "\t\t\t\talignment_type: 0x%02x\n", alignment_type);
#else
	skipBytes(1);
#endif
	numDescriptorBytes -= descriptor_length; parsedDescriptor = True;
...
#ifdef DEBUG_CONTENTS
	  Log.Error(__FILE__, __LINE__, "\t\t\t\tISO_639_language_code: %c%c%c; audio_type: 0x%02x\n",
	...
	Log.Error(__FILE__, __LINE__, "\t\t\t\texternal_clock: %d; clock_accuracy int: %d, exp: %d -> %f ppm\n",
	...
	Log.Error(__FILE__, __LINE__, "\t\t\t\tmaximum_bitrate: %d => %f Mbps\n",
	...
	Log.Error(__FILE__, __LINE__, "\t\t\t\tsb_leak_rate: %d => %f Mbps; sb_size: %d bytes\n",
	...
	Log.Error(__FILE__, __LINE__, "\t\t\t\tprofile_idc: 0x%02x, flags1: 0x%02x, level_idc: 0x%02x, flags2: 0x%02x\n",
	...
	Log.Error(__FILE__, __LINE__, "\t\t\t\tcomponent_tag: %d\n", component_tag);
#else
	skipBytes(1);
#endif
	numDescriptorBytes -= descriptor_length; parsedDescriptor = True;
...
#ifdef DEBUG_CONTENTS
	  Log.Error(__FILE__, __LINE__, "\t\t\t\tISO_639_language_code: %c%c%c",
		  get1Byte(), get1Byte(), get1Byte());
	  u_int8_t typePlusMagazine = get1Byte();
	  Log.Error(__FILE__, __LINE__, "; type: 0x%02x; magazine: %d; page: %d\n",
	...
	for (unsigned i = 0; i < descriptor_length/8; ++i) {
	  ...
	  Log.Error(__FILE__, __LINE__, "\t\t\t\tISO_639_language_code: %c%c%c",
		  get1Byte(), get1Byte(), get1Byte());
	  Log.Error(__FILE__, __LINE__, "; subtitling_type: 0x%02x; composition_page_id: 0x%04x; ancillary_page_id: 0x%04x\n",
...
#ifdef DEBUG_CONTENTS
	  Log.Error(__FILE__, __LINE__, "\t\t\t\tapplication_type: 0x%04x; AIT_version_number: %d\n",
...
#ifdef DEBUG_CONTENTS
	u_int8_t flags = get1Byte();
	Log.Error(__FILE__, __LINE__, "\t\t\t\tflags: 0x%02x", flags);
	if (descriptor_length > 1) {
	  Log.Error(__FILE__, __LINE__, "; extra bytes: ");
	  for (unsigned i = 1; i < descriptor_length; ++i) Log.Error(__FILE__, __LINE__, "0x%02x ", get1Byte());
	}
	Log.Error(__FILE__, __LINE__, "\n");
#else
	skipBytes(descriptor_length);
...
#ifdef DEBUG_CONTENTS
	u_int8_t flags = get1Byte();
	Log.Error(__FILE__, __LINE__, "\t\t\t\tsample_rate_code: %d; bsid: 0x%02x",
		flags>>5, flags&0x1F);
	flags = get1Byte();
	Log.Error(__FILE__, __LINE__, "; bit_rate_code: %d; surround_mode: %d",
		flags>>2, flags&0x03);
	flags = get1Byte();
	Log.Error(__FILE__, __LINE__, "; bsmod: %d; num_channels: %d; full_svc: %d",
		flags>>5, (flags&0x1E)>>1, (flags&0x01));
	if (descriptor_length > 3) {
	  Log.Error(__FILE__, __LINE__, "; extra bytes: ");
	  for (unsigned i = 3; i < descriptor_length; ++i) Log.Error(__FILE__, __LINE__, "0x%02x ", get1Byte());
	}
	Log.Error(__FILE__, __LINE__, "\n");
#else
	skipBytes(descriptor_length);
...
#ifdef DEBUG_CONTENTS
	Log.Error(__FILE__, __LINE__, "\t\t\t\tnumber_of_services: %d\n", number_of_services);
#endif
	if (descriptor_length < number_of_services*6) break;
#ifdef DEBUG_CONTENTS
	for (unsigned i = 0; i < number_of_services; ++i) {
	  Log.Error(__FILE__, __LINE__, "\t\t\t\t\tlanguage: %c%c%c", get1Byte(), get1Byte(), get1Byte());

	  u_int8_t flags = get1Byte();
	  Boolean digital_cc = (flags&0x80) != 0;
	  Log.Error(__FILE__, __LINE__, "; digital_cc %d", digital_cc);
	  if (digital_cc == 0) {
	    Log.Error(__FILE__, __LINE__, "; line21_field: %d", flags&0x01);
	  } else {
	    Log.Error(__FILE__, __LINE__, "; caption_service_number: %d", flags&0x3F);
	  }

	  u_int16_t flags2 = get2Bytes();
	  Log.Error(__FILE__, __LINE__, "; easy_reader: %d; wide_aspect_ratio: %d\n",
...
#ifdef DEBUG_ERRORS
    Log.Error(__FILE__, __LINE__, "MPEG2TransportStreamParser::parseStreamDescriptors() Parsing error left %d bytes unparsed\n",
	    numDescriptorBytes);
#endif
